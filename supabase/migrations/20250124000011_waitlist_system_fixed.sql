-- ============================================================================
-- WAITLIST MANAGEMENT SYSTEM - RESERVE4YOU (FIXED)
-- ============================================================================
-- Complete waitlist system with auto-notify, priority management
-- Fixed FILTER syntax for PostgreSQL compatibility
-- ============================================================================

-- ============================================================================
-- 1. CREATE WAITLIST STATUS TYPE
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_waitlist_priority ON waitlist(priority DESC, created_at);
CREATE INDEX IF NOT EXISTS idx_waitlist_expires ON waitlist(expires_at) WHERE status = 'notified';
...
DROP POLICY IF EXISTS "Users can view waitlist for their tenant locations" ON waitlist;
...
DROP POLICY IF EXISTS "Users can insert waitlist for their tenant locations" ON waitlist;
...
DROP POLICY IF EXISTS "Users can update waitlist for their tenant locations" ON waitlist;
...
DROP POLICY IF EXISTS "Users can delete waitlist for their tenant locations" ON waitlist;
...
-- ============================================================================
-- 5. GET WAITLIST STATISTICS (FIXED)
-- ============================================================================
...
BEGIN
  ...
  -- Count final statuses (for conversion rate)
  SELECT COUNT(*)::INT INTO v_total_final
  FROM waitlist w
  JOIN locations l ON l.id = w.location_id
  WHERE l.tenant_id = p_tenant_id
    AND w.preferred_date = p_date
    AND w.status IN ('converted', 'expired', 'cancelled');
  ...
  SELECT json_build_object(
    ...
    'total_waiting', COALESCE((
      ...
      JOIN locations l ON l.id = w.location_id
      WHERE l.tenant_id = p_tenant_id AND w.preferred_date = p_date AND w.status = 'waiting'
    ), 0),
    'total_notified', COALESCE((
      SELECT COUNT(*)::INT FROM waitlist w
      JOIN locations l ON l.id = w.location_id
      WHERE l.tenant_id = p_tenant_id AND w.preferred_date = p_date AND w.status = 'notified'
    ), 0),
    'total_converted', COALESCE((
      SELECT COUNT(*)::INT FROM waitlist w
      JOIN locations l ON l.id = w.location_id
      WHERE l.tenant_id = p_tenant_id AND w.preferred_date = p_date AND w.status = 'converted'
    ), 0),
    'total_expired', COALESCE((
      SELECT COUNT(*)::INT FROM waitlist w
      JOIN locations l ON l.id = w.location_id
      WHERE l.tenant_id = p_tenant_id AND w.preferred_date = p_date AND w.status = 'expired'
    ), 0),
    'total_cancelled', COALESCE((
      SELECT COUNT(*)::INT FROM waitlist w
      JOIN locations l ON l.id = w.location_id
      WHERE l.tenant_id = p_tenant_id AND w.preferred_date = p_date AND w.status = 'cancelled'
    ), 0),
    'conversion_rate', CASE 
      WHEN v_total_final > 0 THEN
        ROUND(
          (SELECT COUNT(*)::DECIMAL FROM waitlist w
           JOIN locations l ON l.id = w.location_id
           WHERE l.tenant_id = p_tenant_id AND w.preferred_date = p_date AND w.status = 'converted') 
          / v_total_final::DECIMAL * 100, 1
        )
      ELSE 0
    END,
    'avg_wait_time_minutes', COALESCE((
      SELECT ROUND(AVG(EXTRACT(EPOCH FROM (COALESCE(w.notified_at, NOW()) - w.created_at)) / 60))::INT
      FROM waitlist w
      JOIN locations l ON l.id = w.location_id
      WHERE l.tenant_id = p_tenant_id 
        AND w.preferred_date = p_date
        AND w.status IN ('notified', 'converted')
    ), 0),
    'locations', COALESCE((
      SELECT json_agg(
        json_build_object(
...
BEGIN
  ...
  INSERT INTO waitlist (
    location_id, guest_name, guest_phone, guest_email, party_size,
    preferred_date, preferred_time_start, preferred_time_end,
    notes, priority, created_by
  ) VALUES (
    p_location_id, p_guest_name, p_guest_phone, p_guest_email, p_party_size,
    p_preferred_date, p_preferred_time_start, p_preferred_time_end,
    p_notes, p_priority, auth.uid()
  )
...
BEGIN
  ...
  FROM waitlist WHERE id = p_waitlist_id;
...
-- ============================================================================
-- 8. FIND MATCHING WAITLIST (Auto-Notify System)
-- ============================================================================
...
-- ============================================================================
-- 10. GET WAITLIST FOR ALL LOCATIONS
-- ============================================================================
