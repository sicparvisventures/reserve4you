-- ============================================================================
-- CALENDAR SYSTEM - ACTUAL SCHEMA VERSION
-- ============================================================================
-- Only uses columns that actually exist in the database
-- No table_type, no internal_note
-- ============================================================================

-- Clean slate - drop all calendar functions
DO $$ 
DECLARE
    r RECORD;
BEGIN
...
-- ============================================================================
-- 1. GET CALENDAR BOOKINGS (ACTUAL SCHEMA)
-- ============================================================================
...
COMMENT ON FUNCTION get_calendar_bookings IS 'Calendar bookings (actual schema)';
...
COMMENT ON FUNCTION get_table_occupancy IS 'Table occupancy for timeline';
...
BEGIN
  SELECT json_build_object(
    'date', p_date,
    'total_bookings', COALESCE(COUNT(*)::INT, 0),
    'confirmed', COALESCE(COUNT(*) FILTER (WHERE b.status = 'confirmed')::INT, 0),
    'pending', COALESCE(COUNT(*) FILTER (WHERE b.status = 'pending')::INT, 0),
    'cancelled', COALESCE(COUNT(*) FILTER (WHERE b.status = 'cancelled')::INT, 0),
    'no_show', COALESCE(COUNT(*) FILTER (WHERE b.status = 'no_show')::INT, 0),
    'total_guests', COALESCE(SUM(b.number_of_guests)::INT, 0),
    'locations', COALESCE((
      SELECT json_agg(
        json_build_object(
          'id', l.id,
...
COMMENT ON FUNCTION get_calendar_stats IS 'Calendar statistics for dashboard';
...
BEGIN
  ...
  IF v_booking.table_id IS NOT NULL THEN
    ...
    IF v_conflicts > 0 THEN
      RETURN json_build_object('success', false, 'error', 'Time slot conflict', 'conflicts', v_conflicts);
    END IF;
  END IF;
...
GRANT EXECUTE ON FUNCTION get_calendar_bookings(UUID, DATE, DATE) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION get_table_occupancy(UUID, DATE) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION get_calendar_stats(UUID, DATE) TO authenticated, anon;
...
BEGIN
  ...
  IF v_test_location_id IS NOT NULL THEN
    ...
    SELECT json_array_length(v_result) INTO v_count;
    RAISE NOTICE '✅ get_calendar_bookings: % booking(s)', COALESCE(v_count, 0);
    
    SELECT get_calendar_stats(v_test_tenant_id, CURRENT_DATE) INTO v_result;
    RAISE NOTICE '✅ get_calendar_stats: % booking(s) today', (v_result->>'total_bookings');
    
    SELECT get_table_occupancy(v_test_location_id, CURRENT_DATE) INTO v_result;
    SELECT json_array_length(v_result) INTO v_count;
    RAISE NOTICE '✅ get_table_occupancy: % table(s)', COALESCE(v_count, 0);
    
    RAISE NOTICE '';
    RAISE NOTICE '🎉 ALL CALENDAR FUNCTIONS WORKING!';
  ELSE
    RAISE NOTICE '⚠️  No locations (OK for new setup)';
  END IF;
END $$;

SELECT 
  '✅ CALENDAR READY!' as status,
  COUNT(*)::INT as functions
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE p.proname IN (
  'get_calendar_bookings', 'get_table_occupancy', 'get_calendar_stats',
  'update_booking_time', 'assign_table_to_booking', 
  'check_booking_conflicts', 'bulk_update_booking_status'
)
AND n.nspname = 'public';
