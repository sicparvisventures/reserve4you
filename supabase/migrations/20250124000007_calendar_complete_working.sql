-- ============================================================================
-- CALENDAR SYSTEM - COMPLETE WORKING VERSION
-- ============================================================================
-- This is the FINAL working version that fixes all RPC and permission issues
-- ============================================================================
...
DECLARE
    r RECORD;
BEGIN
    -- Drop ALL calendar functions regardless of signature
    FOR r IN 
      ...
      json_build_object(
        ...
        'table_info', CASE 
          WHEN t.id IS NOT NULL THEN json_build_object(
            ...
            'seats', t.seats,
            'table_type', COALESCE(t.table_type, 'standard')
          )
          ELSE NULL
        END,
...
COMMENT ON FUNCTION get_calendar_bookings IS 'Returns bookings for calendar view with all details';

-- ============================================================================
-- 2. GET TABLE OCCUPANCY (for timeline view)
-- ============================================================================
...
COMMENT ON FUNCTION get_table_occupancy IS 'Returns table occupancy for timeline view';

-- ============================================================================
-- 3. GET CALENDAR STATS (for dashboard widget)
-- ============================================================================
...
COMMENT ON FUNCTION get_calendar_stats IS 'Returns calendar statistics for dashboard widget';

-- ============================================================================
-- 4. UPDATE BOOKING TIME (for drag & drop)
-- ============================================================================
...
BEGIN
  ...
  IF v_test_location_id IS NOT NULL THEN
    ...
    RAISE NOTICE 'get_calendar_bookings test: %', v_result;
    
    -- Test get_calendar_stats
    SELECT get_calendar_stats(v_test_tenant_id, CURRENT_DATE) INTO v_result;
    RAISE NOTICE 'get_calendar_stats test: %', v_result;
    
    -- Test get_table_occupancy
    SELECT get_table_occupancy(v_test_location_id, CURRENT_DATE) INTO v_result;
    RAISE NOTICE 'get_table_occupancy test: %', v_result;
    
    RAISE NOTICE '✅ All calendar functions tested successfully!';
  ELSE
    RAISE NOTICE '⚠️  No locations found for testing';
  END IF;
END $$;

-- ============================================================================
-- ✅ CALENDAR SYSTEM COMPLETE AND TESTED
-- ============================================================================
