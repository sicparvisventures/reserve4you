...
export function RotatingLogoMobile() {
  ...
  return (
    <Link href="/" className="flex items-center shrink-0 group">
      <div className="flex items-center gap-1">
        <span className="text-base font-bold text-foreground transition-colors duration-300 group-hover:text-primary/80">
          Reserve
        </span>
        <div className="relative inline-flex items-center overflow-hidden rounded-lg bg-primary/10 px-1.5 py-0.5 border border-primary/20">
          <AnimatePresence mode="wait">
            <motion.span
              key={currentWordIndex}
              initial={{ y: 20, opacity: 0 }}
              ...
              className="text-base font-bold text-primary whitespace-nowrap"
            >
              {rotatingWords[currentWordIndex]}
            </motion.span>
          </AnimatePresence>
